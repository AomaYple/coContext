...
namespace coContext {
    ...
    [[nodiscard]] auto accept4(std::int32_t socket, sockaddr *address, std::uint32_t *addressLength, std::int32_t flags)
        -> AsyncWaiter;

    [[nodiscard]] auto connect(std::int32_t socket, const sockaddr *address, std::uint32_t addressLength)
        -> AsyncWaiter;
}    // namespace coContext
