...
namespace coContext {
    ...
    [[nodiscard]] auto installDirect(std::int32_t directFileDescriptor, bool isSetCloseOnExec = true)
        -> internal::AsyncWaiter;
    ...
    [[nodiscard]] auto multipleAcceptDirect(std::move_only_function<auto(std::int32_t)->void> action,
                                            ...
                                            socklen_t *addressLength, std::int32_t flags = {}) -> Task<>;
    ...
    [[nodiscard]] auto fileStatus(std::int32_t directoryFileDescriptor, std::string_view path, std::int32_t flags,
